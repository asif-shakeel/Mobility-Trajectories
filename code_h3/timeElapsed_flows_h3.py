...
# -*- coding: utf-8 -*-
"""
PMCM mapping toolkit (H3 version)
---------------------------------
H3-based version of pmcm_map.py — identical logic and visualization,
but using Uber’s H3 hex indices (resolution 7) instead of geohashes.

It loads the H3 corridor manifest, builds per-bin OD matrices,
computes netflows, mean flows, mean node accumulations, and
renders all Folium layers (edges, hubs, metro edges, mean-node
accumulation, etc.) with full legend and layer toggles.
"""
...
TIME_RES_MIN = 30
SPAN_START   = "2025-06-01 09:00:00"
SPAN_END     = "2025-06-01 09:30:00"
PEP_SPAN_START   = "2025-06-01 00:00:00"
PEP_SPAN_END     = "2025-06-15 00:00:00"

NETFLOWS_PERCENTILE =0
MIN_COUNT = 1.0
DROP_DIAGONAL = True
...
def export_edges_map(
    ...
    *,
    ...
    """Render edges, hubs, and node overlays as a Folium map."""
    if not HAS_FOLIUM:
    ...
    if edges is not None and not edges.empty:
        ...
        # round to cleaner min/max for colorbar labels
        evmin_disp = np.floor(evmin * 10) / 10
        evmax_disp = np.ceil(evmax * 10) / 10
        cmap_edges = cmb.LinearColormap(
            ["#f5e6ff", "#9c27b0", "#4a0072"], vmin=evmin_disp, vmax=evmax_disp
        )
        ...
        fg_edges.add_to(m)
        # cmap_edges.caption = f"Edge intensity ({evmin_disp:.1f}–{evmax_disp:.1f})"
        cmap_edges.caption = f"Edge trip-count"
        m.get_root().html.add_child(
            folium.Element(
                f'<div style="position:fixed;bottom:20px;right:20px;z-index:9999;">{cmap_edges._repr_html_()}</div>'
            )
        )
    ...
    if metro_edges and (metro_flows or {}):
        ...
        if flows_lookup:
            ...
            mvmin, mvmax = np.floor(m_vals.min()), np.ceil(m_vals.max())
        else:
            mvmin = mvmax = 0.0
        cmap_metro = cmb.LinearColormap(
            colors=["#fff5e6", "#ff9900", "#b35900"], vmin=mvmin, vmax=mvmax
        )
        fg_metro = folium.FeatureGroup(name="Hub↔Hub Metro Edges", show=bool(metro_show))
        ...
        # cmap_metro.caption = "Metro (Hub↔Hub) edge trip-count"
        m.get_root().html.add_child(
            folium.Element(
                f'<div style="position:fixed;bottom:80px;right:20px;z-index:9999;">{cmap_metro._repr_html_()}</div>'
            )
        )
    ...
    if mean_node_accum:
        ...
        cmap_mean = cmb.LinearColormap(["#2166ac", "#f7f7f7", "#b2182b"],
        ...
        m.get_root().html.add_child(
            folium.Element(
                f'<div style="position:fixed;bottom:80px;left:20px;z-index:9999;">{cmap_mean._repr_html_()}</div>'
            )
        )
    ...
    if node_intensity:
        ...
        vmin_disp = np.floor(vmin); vmax_disp = np.ceil(vmax)
        ...
        m.get_root().html.add_child(
            folium.Element(
                f'<div style="position:fixed;bottom:20px;left:20px;z-index:9999;">{cmap_nodes._repr_html_()}</div>'
            )
        )
...
# =========================
# Folium map (identical to pmcm_map.py)
# =========================
# For brevity here, reuse your working export_edges_map(), compute_mean_direct(),
# split_inward_outward_complement(), compute_mean_node_accumulation() from pmcm_map.py
# (they are unchanged except for “geohash”→“h3” field names).
# Paste those sections verbatim from your stable geohash version.
...
def main():
    ...
    if RUN_NETFLOWS:
        ...
        if MAKE_MAPS:
            ...
            export_edges_map(
                edges_net, cells,
                title=f"Net flows (H3 r{H3_RES}) — {t1:%Y-%m-%d %H:%M} → {t2:%Y-%m-%d %H:%M}",
            ...
            # export_edges_map(
            #     edges_net, cells,
            #     title=f"Net trip-counts (H3 r{H3_RES}) — {t1:%Y-%m-%d %H:%M} → {t2:%Y-%m-%d %H:%M}",
            #     node_counts=node_counts,
            #     node_intensity=(node_intensity if SHOW_NODE_INTENSITY else None),
            #     hubs=MANIFEST_HUBS,
            #     metro_edges=MANIFEST_METRO_EDGES,
            ...
            #     metro_show=False,
            # )
    # --- Mean / Inward / Outward / Netflow maps ---
    # (identical to pmcm_map.py; paste corresponding block)
    # ...
    ...
    if RUN_MEAN_EDGE_IN_OUT:
        ...
        # print([e for e in edges_dt_pos if e[0] == "8644c1a1fffffff"])

        # # Edge sets from manifest
        # edges_dt_pos = set(
        #     (r.start_h3, r.end_h3)
        ...
        export_edges_map(
            df_inward, cells,
            # title=f"Mean edge flows (δT = +1, inward) — {t1:%Y-%m-%d %H:%M} → {t2:%Y-%m-%d %H:%M}",
            title=f"Edge trip-count (δT = +1, inward) — {t1:%Y-%m-%d %H:%M} → {t2:%Y-%m-%d %H:%M}",
            hubs=MANIFEST_HUBS,
        ...
        # export_edges_map(
        #     df_inward, cells,
        #     title=f"Edge trip-counts (δT = +1, inward) — {t1:%Y-%m-%d %H:%M} → {t2:%Y-%m-%d %H:%M}",
        #     hubs=MANIFEST_HUBS,
        #     metro_edges=list(metro_dt_pos | metro_dt_zero),
        #     metro_flows=metro_mean_in,
        #     edges_layer_name="Inward edges",
        #     edges_show=True,
        #     metro_layer_name="Metro (mean flows)",
        #     metro_show=False,
        #     outfile=html_in,
        ...
        # export_edges_map(
        #     df_outward, cells,
        #     title=f"Edge trip-counts (δT = −1, outward) — {t1:%Y-%m-%d %H:%M} → {t2:%Y-%m-%d %H:%M}",
        #     hubs=MANIFEST_HUBS,
        #     metro_edges=list(metro_dt_neg | metro_dt_zero),
        #     metro_flows=metro_mean_out,
        #     edges_layer_name="Outward edges",
        #     edges_show=True,
        #     metro_layer_name="Metro (mean flows)",
        #     metro_show=False,
        #     outfile=html_out,
